...
class Chat{
	public static function login($name,$email,$uid){
		if(!$name || !$email){
	...
	public static function submitChat($chatText,$uid){
		if(!$_SESSION['user']){
	...
	public static function getUsers(){
		...
		$result = DB::query('SELECT * FROM webchat_users ORDER BY name ASC LIMIT 5');
	...
	public static function getChats($lastID){
		...
		$result = DB::query('SELECT * FROM webchat_lines WHERE id > '.$lastID.' and DATEDIFF(NOW(), ts) <= 20');
